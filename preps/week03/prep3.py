"""CSC111 Winter 2026 Prep 3: Programming Exercises
...
you can turn these checks into doctests for each function (recommended!).
...
def formula(n: int) -> float:
    """Return the value given by Definition 1 in prep3_functions.pdf.

    Preconditions:
        - n >= 0
    """
    return formula(n - 1) + (4 * (-1) ** n) / (2 * n + 1) if n > 0 else 4.0


@check_contracts
def formula_multiple_args(n: int, a: float, b: float) -> float:
    """Return the value given by Definition 2 in prep3_functions.pdf.

    Preconditions:
        - n >= 0
    """
    return a * formula_multiple_args(n - 1, a, b) + b if n > 0 else 0.0


def formula_double_recursion(n: int, m: int) -> int:
    """Return the value given by Definition 3 in prep3_functions.pdf.
...
def create_list1(n: int) -> list[int]:
    """Return the value given by Definition 4 in prep3_functions.pdf.

    Preconditions:
        - n >= 0
    """
    return 2 * create_list1(n - 1) + [n] if n > 0 else [0]


@check_contracts
def create_list2(n: int, m: int) -> list:
    """Return the value given by Definition 5 in prep3_functions.pdf.
