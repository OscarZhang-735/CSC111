...
def test_len_linkedlist():
    linky = build_sample_linked_list()
    assert len(linky) == 4


def test_count_linkedlist():
    linky = build_sample_linked_list()
    assert linky.count(111) == 2
    assert linky.count(9000) == 1
    assert linky.count(-5) == 1


def test_index_linkedlist():
    linky = build_sample_linked_list()
    assert linky.index(111) == 0
    assert linky.index(9000) == 2


def test_setitem_linkedlist():
    linky = build_sample_linked_list()
    linky[0] = 1
    assert linky[0] == 1
