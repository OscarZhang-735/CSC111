...
Module Description
==================
This module contains the Tree class we developed in last week's lectures, as well as
...
class Tree:
    ...
    def insert_sequence(self, items: list) -> None:
        ...
        new_subtree = Tree(first, [])
        self._subtrees.append(new_subtree)

        new_subtree.insert_sequence(rest)

    def find_matches(self, answers: list[bool]) -> list[str]:
        """Return a list of all matching animals based on the given answers.

        answers represents the path of True/False decisions from this tree.
        """
        # Base case: no more answers → collect all animals below this node
        if answers == []:
        ...
        for subtree in self._subtrees:
            if subtree._root == first:
                matches.extend(subtree.find_matches(rest))

        return matches

    def _collect_leaves(self) -> list[str]:
        """Return all leaf values under this tree."""
        if self._subtrees == []:
            return [str(self._root)]

        leaves = []
        for subtree in self._subtrees:
            leaves.extend(subtree._collect_leaves())
        return leaves


# TODO: Add _length attribute.
################################################################################
...
def run_animal_guesser(animal_file: str) -> None:
    ...
    matches = decision_tree.find_matches(answers)

    if matches == []:
        print("No animals found.")
    elif len(matches) == 1:
        print(f"1 animal found exactly: {matches[0]}")
    else:
        print("Some possible animals found:")
